...
async function startServer() {
  ...
  app.get("/api/weather", async (req, res) => {
    try {
      ...
      let apiKey =
        process.env.OPENWEATHER_API_KEY || "712ffaa5e4cbf434203f2402e54f6b7d";
      if (apiKey) {
        apiKey = apiKey
          .trim()
          .replace(/^["']|["']$/g, "")
      ...
      if (!apiKey || apiKey === "YOUR_API_KEY_HERE" || apiKey === "") {
        ...
        return res.status(401).json({
          ...
          message:
            "OpenWeatherMap API key is not configured. Please set the OPENWEATHER_API_KEY environment variable in your secrets or .env file.",
        });
      }
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          return res.status(401).json({
            ...
            message:
              "The configured OpenWeatherMap API key is invalid or not yet activated. Please check your key or try again in a few minutes.",
          });
        }
        return res.status(response.status).json({
          error: "API_ERROR",
          message:
            data.message || "City not found — check the spelling and try again",
        });
      }
  ...
  app.get("/api/debug-weather", (req, res) => {
    try {
      ...
      res.json({
        ...
        key_end: cleanedKey
          ? "..." + cleanedKey.substring(cleanedKey.length - 4)
          : "none",
        contains_quotes:
          rawKey.startsWith('"') ||
          rawKey.startsWith("'") ||
